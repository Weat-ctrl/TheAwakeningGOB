import * as THREE from './lib/three/three.module.js';
   ...
   const aoMap = textureLoader.load(aoMapUrl, () => {
       console.log('AO map loaded successfully');
   }, undefined, (error) => {
   ...
   function addRuin(textureUrl, width, height, positionX, positionZ, rotationY = 0) {
       const ruinMap = textureLoader.load(textureUrl, () => {
           console.log(`Loaded ruin texture: ${textureUrl}`);
       }, undefined, (error) => {
           console.error(`Error loading ruin texture ${textureUrl}:`, error);
       });
       ...
       ruin.position.set(positionX, height / 2, positionZ); // Bottom aligns with floor
   ...
   function addCharacter(x, z, inFront = true) {
       const characterGeometry = new THREE.PlaneGeometry(1, 2);
       const characterMaterial = new THREE.MeshStandardMaterial({ color: inFront ? 0xff0000 : 0x0000ff }); // Red for front, blue for behind
       const character = new THREE.Mesh(characterGeometry, characterMaterial);
       character.position.set(x, 1, z + (inFront ? 0.1 : -0.1)); // Adjust z for front/behind
   ...
   // Add test characters
   addCharacter(0, 0, true); // Red character in front
   addCharacter(0, 2, false); // Blue character behind
